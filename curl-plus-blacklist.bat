...
REM --------------------------------------------------
echo 19) add to blacklist (admin) - UNIQ FEATURE
curl -X POST http://localhost:10001/blacklist --header "Content-Type: application/json" --header "Authorization: Basic kienboec-ppbToken" -d "{ \"Title\": \"Last_Christmas\", \"Url\": \"http://y2u.be/E8gmARGvPlI\"}"
curl -X POST http://localhost:10001/blacklist --header "Content-Type: application/json" --header "Authorization: Basic kienboec-ppbToken" -d "{ \"Title\": \"I_sing_a_Liad_fuer_Di\", \"Url\": \"http://y2u.be/U0_UYW5Y4cM\"}"
echo.
echo.


REM --------------------------------------------------
echo 20) add to blacklist (should fail) - UNIQ FEATURE
curl -X POST http://localhost:10001/blacklist --header "Content-Type: application/json" --header "Authorization: Basic altenhof-ppbToken" -d "{ \"Title\": \"ModernTalking\", \"Url\": \"http://y2u.be/4kHl4FoK1Ys\"}"
echo.
echo.

REM --------------------------------------------------
echo 21) try to add blacklist-title (should fail) - UNIQ FEATURE
curl -X POST http://localhost:10001/lib --header "Content-Type: application/json" --header "Authorization: Basic altenhof-ppbToken" -d "{\"Name\": \"Last_Christmas\", \"Url\": \"http://y2u.be/E8gmARGvPlI\", \"Rating\": 5, \"Genre\": \"Pop\"}"
curl -X POST http://localhost:10001/playlist --header "Content-Type: application/json" --header "Authorization: Basic kienboec-ppbToken" -d "{\"Name\": \"Last_Christmas\"}"
echo.
echo.

REM --------------------------------------------------
echo 22) check playlist - UNIQ FEATURE
curl -X GET http://localhost:10001/playlist
echo.
echo.

REM --------------------------------------------------
echo 23) set actions to simulate a draw
